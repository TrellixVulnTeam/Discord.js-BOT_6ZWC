...
async function handler(args, opts) {
    ...
    if (opts.recursive && opts.selectedProjectsGraph) {
        ...
        await recursivePublish_1.default(pkgs, {
            ...opts,
            workspaceDir: (_a = opts.workspaceDir, (_a !== null && _a !== void 0 ? _a : process.cwd())),
        });
        return;
    }
    if (args.length && args[0].endsWith('.tgz')) {
        await run_npm_1.default(opts.npmPath, ['publish', ...args]);
        return;
    }
    ...
    await fakeRegularManifest({
        ...
        const { status } = await run_npm_1.default(opts.npmPath, ['publish', ...opts.argv.original.slice(1)]);
        _status = status;
    });
    if (_status !== 0) {
